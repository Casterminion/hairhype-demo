/*
  # Create Blog Content Images Storage Bucket

  1. Storage
    - Create `blog-content-images` storage bucket for inline blog image uploads
  ...
  3. Security
    ...
    - Restrict to image file types only (JPEG, PNG, GIF, WebP)
*/

-- Create storage bucket for blog content images
...
DROP POLICY IF EXISTS "Public can view blog content images" ON storage.objects;
...
-- Allow public read access to blog content images
CREATE POLICY "Public can view blog content images"
ON storage.objects
FOR SELECT
TO public
